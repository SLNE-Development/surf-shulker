...
import dev.slne.surf.shulker.api.property.JAVA_PATH
...
abstract class AbstractRuntimeFactory<S : Service>(
    ...
    protected suspend fun environment(service: S): PlatformParameters {
        ...
        environment["hostname"] set service.hostname
        environment["port"] = service.port.toString()
        environment["agent-port"] set Agent.config.port
        environment["agent-hostname"] set Agent.runtime.detectedLocalAddress
        environment["service-name"] set service.name
        environment["velocity-proxy-token"] set Agent.securityProvider.proxySecurityToken
        environment["file-name"] set service.group.applicationPlatformFile.name
        ...
        environment["velocity-use"] set
                groups.stream().anyMatch { velocityPlatforms.contains(it.platform.name) }
        environment["bungeecord-use"] set groups.stream()
